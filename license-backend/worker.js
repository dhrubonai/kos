/**
 * KOS License Server — Cloudflare Worker
 * ========================================
 * Self-hosted license generation + validation backend for the KOS app.
 *
 * Bindings required at deploy time:
 *   - KV namespace  : KOS_LICENSES
 *   - Secret        : ADMIN_PASSWORD   (set via `wrangler secret put` or API)
 *
 ...
 *   POST /api/admin/licenses   {plan, days, note, quantity, deviceLock}
...
function normKey(k) {
  return String(k || "").toUpperCase().trim().replace(/\s+/g, "").replace(/[^0-9A-Z-]/g, "");
}
...
async function handleValidate(body, env) {
  ...
  const raw = await env.KOS_LICENSES.get("lic:" + key);
  if (!raw) return json({ valid: false, message: "Invalid license key" });
  let lic; try { lic = JSON.parse(raw); } catch { return json({ valid: false, message: "Corrupt license record" }); }
  if (lic.revoked) return json({ valid: false, message: "License has been revoked" });

  if (!lic.device) {
    lic.device = device;
    lic.activatedAt = now();
    lic.expiresAt = lic.days == null ? null : lic.activatedAt + lic.days * 86400000;
    await env.KOS_LICENSES.put("lic:" + key, JSON.stringify(lic));
    await env.KOS_LICENSES.put("device:" + device, key);
...
async function handleConnect(body, env) {
  ...
  const key = await env.KOS_LICENSES.get("device:" + device);
  if (!key) return json({ bound: false, active: false });
  const raw = await env.KOS_LICENSES.get("lic:" + key);
  if (!raw) return json({ bound: false, active: false });
...
async function requireAdmin(request, env) {
  ...
  const ok = await env.KOS_LICENSES.get("session:" + token);
  return ok === "admin" ? token : null;
}
...
async function handleAdminLogin(body, env) {
  ...
  await env.KOS_LICENSES.put("session:" + token, "admin", { expirationTtl: 60 * 60 * 12 });
...
async function handleCreateLicense(body, env) {
  ...
  for (let i = 0; i < quantity; i++) {
    ...
    if (await env.KOS_LICENSES.get("lic:" + key)) { i--; continue; }
    const rec = {
      key,
      plan,
    ...
    await env.KOS_LICENSES.put("lic:" + key, JSON.stringify(rec));
    await env.KOS_LICENSES.put("index:" + String(now()).padStart(15, "0") + ":" + key, key);
    made.push(rec);
  }
...
async function handleUpdateLicense(body, env) {
  const key = normKey(body.key);
  const raw = await env.KOS_LICENSES.get("lic:" + key);
  if (!raw) return json({ ok: false, message: "Key not found" }, 404);
  const lic = JSON.parse(raw);
  ...
  switch (action) {
    ...
    case "unbind":
      if (lic.device) await env.KOS_LICENSES.delete("device:" + lic.device);
    ...
    case "delete":
      await env.KOS_LICENSES.delete("lic:" + key);
      if (lic.device) await env.KOS_LICENSES.delete("device:" + lic.device);
...
async function handleList(url, env) {
  ...
  let cursor;
  do {
    const page = await env.KOS_LICENSES.list({ prefix: "index:", cursor, limit: 500 });
    for (const k of page.keys) {
      const key = k.name.slice("index:".length + 16);
      const raw = await env.KOS_LICENSES.get("lic:" + key);
      if (!raw) continue;
      const lic = JSON.parse(raw);
      const isActive = !lic.revoked && (lic.expiresAt == null || now() <= lic.expiresAt);
      if (status === "active" && !isActive) continue;
      if (status === "revoked" && !lic.revoked) continue;
      if (status === "expired" && (lic.expiresAt == null || now() <= lic.expiresAt)) continue;
      if (status === "unused" && lic.device) continue;
      if (q && !lic.key.includes(q) && !(lic.note || "").toUpperCase().includes(q) && !(lic.device || "").toUpperCase().includes(q)) continue;
      out.push(lic);
...
async function handleSummary(env) {
  let total = 0, active = 0, unused = 0, revoked = 0, expired = 0, bound = 0;
  let cursor;
  do {
    const page = await env.KOS_LICENSES.list({ prefix: "index:", cursor, limit: 500 });
    for (const k of page.keys) {
      ...
      const lic = JSON.parse(raw);
      total++;
      if (lic.revoked) { revoked++; continue; }
      if (lic.expiresAt != null && now() > lic.expiresAt) { expired++; continue; }
      if (!lic.device) { unused++; continue; }
      active++; bound++;
    }
    cursor = page.list_complete ? undefined : page.cursor;
  } while (cursor);
  return json({ ok: true, total, active, unused, revoked, expired, bound });
}
